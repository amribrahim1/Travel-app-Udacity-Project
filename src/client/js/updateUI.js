...
const updateUI = async (travelDate,endDate) => {
    ...
    try {
        ...
        $('#departure').html(travelDate);   // update travel date
        $('#return').html(endDate);     // update end date
        // update the country's flag 
        document.getElementById('ModalLabel').innerHTML = `<img src="https://www.countryflags.io/${allData.city.countryCode}/flat/64.png">  ${allData.city.toponymName}, ${allData.city.countryName}`;
        ...
        // update forecast weather for the accommodation days of the city
        ...
        // update list of hotels in the city for select from them
        for (const hotel of allData.hotels) {
            document.getElementById('hotelSelect').innerHTML += `<option value="${hotel.hotelName}">${hotel.hotelName}</option>`
        }
